...
export default function ModalEditarCor({ isOpen, onClose, cor, onSucesso }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE_URL}/api/cores/${cor.id}`, {
        method: 'PUT',
        headers: {
      ...
      if (res.ok) {
        ...
        alert(err.message || 'Erro ao atualizar cor');
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[70] p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm overflow-hidden border-t-8 border-[#FF9B54]">
        <div className="p-4 bg-gray-50 border-b flex justify-between items-center">
          <h3 className="text-lg font-black text-[#2A3240] uppercase">Editar Cor</h3>
